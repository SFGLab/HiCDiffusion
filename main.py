...
def main(only_predict=None):
    ...
    if(predict):
        if(only_predict == None):
            ...
            model = Interaction3DPredictor.load_from_checkpoint(only_predict)
        ...
        for prediction_batch, real_data_batch in tqdm(zip(predictions, iter(genomic_data_module.predict_dataloader()))):
            for prediction, real_data, real_chr, real_pos, real_end in zip(prediction_batch, real_data_batch[1], real_data_batch[2][0], real_data_batch[2][1], real_data_batch[2][2]):
                if(produce_heatmaps):
                    ...
                    plt.imshow(prediction/datasets.scaling_factor, cmap='binary', interpolation='nearest')
                    plt.subplot(1, 3, 2)
                    plt.gca().set_title('Real')
                    plt.imshow(real_data/datasets.scaling_factor, cmap='binary', interpolation='nearest')
                    plt.subplot(1, 3, 3)
                    plt.gca().set_title('Difference')
                    plt.imshow((real_data-prediction)/datasets.scaling_factor, cmap='binary', interpolation='nearest')
                    plt.colorbar()
                    plt.tight_layout()
